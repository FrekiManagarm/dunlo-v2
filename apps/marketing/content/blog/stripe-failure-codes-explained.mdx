---
title: "Stripe Failure Codes: Email and Retry Playbook"
description: "Learn what card_expired, insufficient_funds, do_not_honor, and card_declined mean in Stripe, plus the recovery email and retry timing to use next."
date: "2026-05-18"
published: true
...
keywords:
  [
    ...
    "stripe card_expired email",
    "stripe insufficient_funds recovery",
...
The customer hit a balance, credit, transaction amount, or card velocity limit. Stripe surfaces this as `card_velocity_exceeded`, and some founders search for it as `card_velocity_exceed`. Treat both phrases as the same operational problem: the card is not necessarily dead, but another immediate retry is likely to fail.
...
For the full recovery workflow around failed invoices, see the [Stripe failed payments guide](/stripe-failed-payments).
