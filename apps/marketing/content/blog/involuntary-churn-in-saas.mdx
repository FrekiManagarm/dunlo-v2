...
| --- | --- | --- | --- |
| Expired card | `expired_card` | The customer needs to update card details. | Send an immediate update link. |
...
Use Stripe's [test-card and decline-code reference](https://docs.stripe.com/testing) when validating your failure-code handling.
...
4. Verify that saved cards are attached to the Stripe Customer, then monitor Stripe's [automatic card updates](https://docs.stripe.com/payments/cards/overview) instead of treating card updating as a manual toggle.
