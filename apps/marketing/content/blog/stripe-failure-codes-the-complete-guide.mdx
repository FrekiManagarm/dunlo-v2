...
| `card_velocity_exceeded` | The card hit a velocity, amount, or limit rule. | Wait at least 24 hours before retrying and avoid tight retry loops. | "Your bank limited this charge." |
...
### What does `card_velocity_exceeded` mean in Stripe?

`card_velocity_exceeded` means the card hit a bank, issuer, amount, frequency, or card-limit rule. The safest next step is to wait before retrying, explain that the bank limited the charge, and offer a secure billing link for another card.
...
The customer hit a balance, credit, transaction amount, or card velocity limit. Some founders search for this as `card_velocity_exceed`, but Stripe's decline code is `card_velocity_exceeded`.

Recommended copy:

> Your bank limited this charge. You can approve it with the bank or switch to another card here: [secure billing link].

Recommended timing: wait at least 24 hours before retrying, avoid repeated attempts unless the customer says the limit is resolved, and move high-value accounts into manual follow-up if the same code appears twice.
...
| `card_velocity_exceeded` | Bank limit explanation | Wait 24h+ | Avoid tight retry loops |
