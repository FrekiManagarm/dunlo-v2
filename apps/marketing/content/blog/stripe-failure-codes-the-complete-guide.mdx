...
If you want to compare your own numbers before fixing the workflow, use the free [Stripe failed payment benchmark](/benchmark). For the full recovery system around failed invoices, read the [Stripe failed payments guide](/stripe-failed-payments).
...
| Customer action required | `authentication_required`, `incorrect_cvc`, `incorrect_zip` | The customer needs to confirm or correct something. | Send the right secure action link. |
...
[Dunlo](https://dunlo.io) reads the failure code from Stripe, routes to the right email sequence, handles timing, tracks recovered revenue, and flags high-value accounts for founder review. It is free during beta.
