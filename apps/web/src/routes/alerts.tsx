        ...
        <nav className="flex-1 px-3 py-4 space-y-px">
          {NAV_ITEMS.map(({ icon: Icon, label, to, ...rest }) => (
            <Link
              key={label}
              to={to}
              className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition-all ${"active" in rest && rest.active
                ? "bg-dunlo/[0.07] font-semibold text-dunlo-deep"
                : "font-medium text-zinc-500 hover:bg-zinc-50 hover:text-zinc-800"
              }`}
            >
              <Icon size={15} className={"active" in rest && rest.active ? "text-dunlo" : ""} />
              {label}
            </Link>
          ))}
        </nav>
